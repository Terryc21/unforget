# UNFORGET - Single Source of Truth for Deferred Work

<!-- unforget-format: v1 -->

> [!IMPORTANT]
> **Format notice for editors and AI assistants.** This file is maintained by the [unforget skill](https://github.com/Terryc21/unforget) and is the project's only ledger for deferred work. Adding rows by hand is fine. Changing the structure will break tooling.
>
...
>
> If a column or section truly needs to change, propose it upstream rather than editing this file directly.

**Last promoted:** 2026-04-15 (build 12 shipped; one P-row demoted, four A-rows fixed).
**Currently shipping toward:** ExampleApp 2.1 (build 13) - App Store submission

> This file is the **only** ledger for deferred work. When a session surfaces something to defer, log it here. Do not split into separate plan files unless the entry needs detail beyond one row - and even then, link the detail file from the row's footnote, don't replace this row.
...
Each section below has a table of deferred items, one per row. Tables display cleanly in:

- **Markdown preview tools** (clearest view): GitHub web, Marked.app, VS Code preview, macOS Quick Look (select file in Finder, press space).
- **Plain terminal** (works too, just less polished). Widen the window if rows wrap.

Each row has a short **Finding** - the one-sentence summary. If a Finding needs more explanation, that detail goes in the **Detail** block below the table, keyed by the row's ID.

### How to add a new row

1. Pick the right section (Paused plans / Session spillover / Audit findings / User-reported).
2. Use the next ID in that section. **Never reuse an ID. Never renumber existing IDs.** Once assigned, an ID points at one row forever, even if that row is later closed.
3. Findings must be 50 characters or fewer. Hard rule, not guideline.
4. No `<br>` tags. No em dashes or double hyphens (use single dash or colon).
5. Fill in all 10 columns.

### Detail block format

1. **Closure pointer first if the row is closed.** Lead with `**CLOSED YYYY-MM-DD: [summary].**`
2. **Body**: history, files, plan refs, context.
3. **Spawn links**: `Spawned-from: <ID>` if this row was created by closing another. `Spawns: <ID>` if closing this row created a follow-up.

For Open rows, skip step 1.
