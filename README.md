...
- **Maturity:** v2.8.0 (backward compatible with v1 ledgers); used through an actual App Store submission cycle in the source project; setup flow specified in detail with two rounds of nondestructive testing.
...
| `/unforget branch` | Spin off a sibling ledger (a sprint, a user-only punch-list, a different repo) when the work genuinely differs from the main one — without losing the pointer back to it. Format v2+. |
