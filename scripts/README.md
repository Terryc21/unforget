...
| `display_prefs.py` | (format v2+) Display-preference resolution: `resolve` applies `flag > registry > default` and reports a `sources` map; `framing` picks the first-run vs re-run lead-in for `--fresh`; `build-patch` turns interview answers into a `registry.py write --merge` payload carrying only answered keys. Fails safe — missing README, bad enum, or garbage threshold all fall back to defaults. | `reference/commands.md` § Display-preference interview; `/unforget list`, `scan`, `list --fresh` |
